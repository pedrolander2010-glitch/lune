...
export const ControlBar: React.FC<ControlBarProps> = ({
  ...
  return (
    <div className="fixed bottom-3 sm:bottom-6 left-1/2 -translate-x-1/2 z-40 max-w-[96vw]">
      <div className="h-16 sm:h-20 px-2 sm:px-6 bg-[#0c0d15]/90 sm:bg-white/5 backdrop-blur-2xl border border-white/15 sm:border-white/10 rounded-2xl sm:rounded-3xl flex items-center justify-center gap-1.5 sm:gap-4 shadow-2xl shadow-black/80">
        {/* Mic toggle */}
        <button
          onClick={onToggleMute}
          className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl transition active:scale-95 flex items-center justify-center border shrink-0 ${
            isMuted
        ...
        <div className="relative shrink-0">
          <div className="flex items-center rounded-xl sm:rounded-2xl overflow-hidden border border-white/10">
            <button
              onClick={onToggleScreenShare}
              className={`h-10 sm:h-12 px-2.5 sm:px-4 transition active:scale-95 flex items-center gap-1.5 sm:gap-2 ${
                isScreenSharing
            ...
            <button
              ...
              className={`px-1.5 sm:px-2 h-10 sm:h-12 border-l border-white/10 transition ${
                isScreenSharing
        ...
        <button
          ...
          className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl transition active:scale-95 flex items-center justify-center border relative shrink-0 ${
            isChatOpen
        ...
        >
          ...
          {unreadChatCount > 0 && !isChatOpen && (
            <span className="absolute top-2 right-2 w-2 h-2 rounded-full bg-indigo-500 shadow-sm shadow-indigo-500 animate-pulse" />
          )}
        </button>
        ...
        >
          ...
          {activeFileTransfersCount > 0 && (
            <span className="absolute top-2 right-2 w-2 h-2 rounded-full bg-emerald-400 shadow-sm shadow-emerald-400 animate-pulse" />
          )}
        </button>
        ...
        <button
          ...
          className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-white/5 hover:bg-white/10 text-emerald-400 border border-white/10 transition active:scale-95 hidden sm:flex items-center justify-center shrink-0"
        ...
        {isInRoom && (
          <button
            onClick={onLeaveRoom}
            className="px-3 sm:px-6 h-10 sm:h-12 rounded-xl sm:rounded-2xl bg-red-500/20 border border-red-500/40 text-red-400 font-bold hover:bg-red-500/30 flex items-center gap-1.5 sm:gap-2.5 shadow-lg shadow-red-500/20 active:scale-95 transition-all text-[11px] sm:text-xs tracking-wider uppercase shrink-0"
